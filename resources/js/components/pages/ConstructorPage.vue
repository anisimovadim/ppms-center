    ...
    <div class="main-block" v-else>
        ...
        >
            ...
            >
                ...
                <textarea :id="`item${index}`" :ref="`text${index}`" class="text"
                          v-if="item.element.type==='txt' && item.editText" v-model="item.element.value"
                          @input="changeText(index)"
                          :style="{height:item.heightText + 'px'}"
                >
                </textarea>
                <div class="text" draggable="false" v-if="item.element.type==='txt' && !item.editText">
                    {{ item.element.value }}
                </div>
            </resize-block>
        </div>
        ...
        <modal-component key="modal" :title="'Изменить блок'" :id="'editBlockModal'" v-if="selectItem >= 0"
                         @actionFrom="changeElement">
...
<script>
import ResizeBlock from "@/components/ui/ResizeBlock.vue";
...
export default {
    ...
    methods: {
        ...
        saveBlock() {
            this.loader = true;
            api.post('/api/auth/create/new', {items: this.items, titleBlock: this.titleBlock})
                .then(res => {
                    this.loader = false;
        ...
        editText(item, event) {
            if (this.selectItem !== -1
                && event.target.classList[0] === 'border-resize'
                && item.element.type === 'txt') {
                item.editText = true;
